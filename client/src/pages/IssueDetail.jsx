  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <p className="mb-6 text-gray-700">{dummyIssue.description}</p>

        <div className="mb-6">
          <h3 className="text-lg font-semibold mb-2">Status Timeline</h3>
          <div className="space-y-3">
            {dummyIssue.timeline.map((step, index) => (
              <TimelineStep key={index} label={step} isActive={step === currentStatus || dummyIssue.timeline.indexOf(currentStatus) > index} />
            ))}
          </div>
        </div>
